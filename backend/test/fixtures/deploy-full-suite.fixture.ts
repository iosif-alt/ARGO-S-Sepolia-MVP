...
export type FullSuiteOptions = {
  tokenName?: string;
  tokenSymbol?: string;
  tokenDecimals?: BigNumber;
  claimTopicLabel?: string;
  aliceInitialBalance?: number;
  bobInitialBalance?: number;
};

export async function deployFullSuiteFixture(options: FullSuiteOptions = {}) {
  ...
  // Defaults preserve the upstream test suite. Project-specific deployments
  // override these values without changing the audited T-REX contracts.
  const tokenName = options.tokenName ?? 'TREXDINO';
  const tokenSymbol = options.tokenSymbol ?? 'TREX';
  const tokenDecimals = options.tokenDecimals ?? BigNumber.from('0');
  ...
  const claimTopics = [ethers.utils.id(options.claimTopicLabel ?? 'CLAIM_TOPIC')];
  ...
  await token.connect(tokenAgent).mint(aliceWallet.address, options.aliceInitialBalance ?? 1000);
  await token.connect(tokenAgent).mint(bobWallet.address, options.bobInitialBalance ?? 500);
